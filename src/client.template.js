...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 两条新建路径共用的「先看有没有别人的待命槽」闸门：
		 * 命中就自己造会话并返回 true，调用方不要再走平台流程（那条会端走别人的槽）。
		 * 由 sessions fiber 注入 —— 换目录那条路在 uiWorkspace fiber 里，拿不到它。
		 */
		let createSeatSessionFresh = null;
...
`;
		...
		function apply(ctx) {
			...
			ctx.inject(["sessions"], (c) => {
				...
				/**
				 * 目标工作区里那个「已经名花有主的空白会话」。
				 *
				 * 平台的 `connectWorkspace()` 会复用「该工作区第一个 blank 会话」（源码写死）。
				 * 而八席的模型里「待命」是一个**正式状态**（ux-design §1.1）—— 多个席同时各自
				 * 持有一个空白槽是常态，不是异常。于是用户在空席点新建时，平台很可能把
				 * **别人的待命槽**端过来，认领规则再把它从原席移走 —— 用户看到的就是
				 * 「我从八席起的这个对话，被挪到别的格子去了」。
				 *
				 * 这永远不该发生：会话换了格子，但那不是用户的意思。所以发起新建前先探一下，
				 * 只要有主就绕开平台的复用（见 createFreshSession）。
				 *
				 * 纯函数（快照 + 绑定表进、席位出），便于离线验证。
				 *
				 * @param workspace - 目标工作区（WorkspaceView）。
				 * @param snapshot - `sessions.list` 快照。
				 * @param bind - 当前席位绑定表。
				 * @returns 持有那个空白会话的席位；没有「有主的空白槽」时返回 null。
				 */
				const ownedBlankIn = (workspace, snapshot, bind) => {
					if (workspace === undefined || snapshot === undefined) return null;
					...
					for (const id of ids) {
						...
						const owner = SEATS.find((seat) => bind[seat.id] === id);
				...
				/**
				 * 绕开平台复用、显式造一个新会话，直接绑到这一席。
				 *
				 * 只在「目标工作区里已经有别人的待命槽」时走这条路；否则仍然走平台的
				 * `startSession()`（复用工作区那唯一的空白槽），平台模型不变。
				 *
				 * 这里**不经过认领**：认领是"猜哪一个才是我的"，而这个是"这一个就是我造的" ——
				 * 不需要猜，也就不会抢到别人头上。
				 */
				const createFreshSession = (seatId, workspaceId) => {
					if (disposed || creatingSeats.has(seatId)) return;
					creatingSeats.add(seatId);
					later(() => creatingSeats.delete(seatId), 8000);
					Promise.resolve(c.sessions.create({ workspaceId }))
						.then((sessionId) => {
							if (disposed || typeof sessionId !== "string") return;
							setSeatBinding(seatId, sessionId);
							lastSeatId = seatId;
							c.sessions.open(sessionId);
							ctx.layout.selectPanel(null);
						})
						.catch((error) => {
							console.warn("[agent-grid] 显式新建会话失败，回退到平台流程：", error);
							if (disposed) return;
				...
				/**
				 * 两条新建路径（空席新建 / 换目录）共用的入口。
				 *
				 * 目标工作区里若有**别人的待命槽**，就自己造一个新会话并返回 true ——
				 * 调用方**不要**再走平台流程，那条会端走别人的槽。
				 *
				 * @returns 是否已经代为处理（true = 调用方就此收工）。
				 */
				const createSeatSessionFreshImpl = (seatId, workspaceId) => {
					if (disposed || workspaceId === undefined) return false;
					const ws = (workspaceListOf() || []).find((item) => item.workspaceId === workspaceId);
					if (ownedBlankIn(ws, c.sessions.list.getSnapshot(), readStore().bind) === null) return false;
					createFreshSession(seatId, workspaceId);
					return true;
				};
				createSeatSessionFresh = createSeatSessionFreshImpl;

				/** 真正发起「这一席新开一个会话」：工作区已定，只差认领 + 交给平台。 */
				const startSeatSessionAt = (seatId, workspace) => {
					// 有主就先自己造 —— 见 ownedBlankIn 的注释（搬别人的格子永远不该发生）。
					if (createSeatSessionFresh !== null && createSeatSessionFresh(seatId, workspace.workspaceId)) return;
				...
				const claimPendingSeat = () => {
					...
					if (pendingSeat !== null) {
						...
						if (Date.now() - pendingSeat.at > CLAIM_TTL_MS) {
							...
							// 已经属于**别的席**的空白槽，不是「平台新开的槽」，是人家的待命位。
							// 把它认领过来 = 把别人的格子搬走（用户原话：从八席起的对话被挪走了）。
							// 走到这里说明发起端没能绕开平台的复用 —— 宁可继续等下一个会话，
							// 也不搬别人的格子；这次新建由 TTL 兜底解锁，用户再点一次即可。
							const claimedByOther = SEATS.find((seat) => seat.id !== pendingSeat.seatId
								&& readStore().bind[seat.id] === snapshot.current);
							if (claimedByOther !== undefined) return;
				...
				return () => {
					...
					if (createSeatSessionFresh === createSeatSessionFreshImpl) createSeatSessionFresh = null;
				};
			});
			...
			ctx.inject(["uiWorkspace", "workspaces"], (w) => {
				...
				switchSeatDirectory = async (seatId) => {
					try {
						...
						// 目标工作区里若已经有**别人的待命槽**，平台的 startSession() 会端走它
						// （connectWorkspace 复用「该工作区第一个 blank」）—— 那等于把别人的
						// 格子搬走。这种时候自己造一个，绕开复用（见 createSeatSessionFresh）。
						if (createSeatSessionFresh !== null && createSeatSessionFresh(seatId, workspaceId)) return;
